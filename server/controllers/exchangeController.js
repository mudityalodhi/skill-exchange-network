...
const sendExchangeRequest = async (req, res) => {
  ...
  if (req.user.credits < creditCost) {
    return res
      .status(400)
      .json({ success: false, message: "Insufficient SEN credits." });
  }
  ...
  res
    .status(201)
    .json({ success: true, message: "Exchange request sent!", exchange });
};
...
const respondToExchange = async (req, res) => {
  ...
  if (exchange.status !== "pending") {
    return res
      .status(400)
      .json({ success: false, message: "Exchange already responded to." });
  }

  if (action === "accept") {
    // Deduct credits from sender
    await User.findByIdAndUpdate(exchange.sender._id, {
      $inc: { credits: -exchange.creditCost },
    });
...
const completeExchange = async (req, res) => {
  ...
  if (
    ![exchange.sender.toString(), exchange.receiver.toString()].includes(
  ...
  if (exchange.status !== "accepted") {
    return res
      .status(400)
      .json({ success: false, message: "Exchange must be accepted first." });
  }
...
const getExchangeById = async (req, res) => {
  ...
  const isParticipant = [
    exchange.sender._id.toString(),
    exchange.receiver._id.toString(),
