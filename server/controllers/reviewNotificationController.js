...
const createReview = async (req, res) => {
  ...
  const isParticipant = [exchange.sender.toString(), exchange.receiver.toString()].includes(
  ...
  if (existing) {
    return res.status(400).json({ success: false, message: 'Already reviewed this exchange.' });
  }
...
const getUserReviews = async (req, res) => {
  ...
  const total = await Review.countDocuments({ reviewee: req.params.userId, isVisible: true });
